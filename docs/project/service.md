...
- 动态容器网络：上层 `start` 时 `docker network create` 一个专属该实例的动态隔离网络（`${INSTANCE_NAME}_net`），把底层容器 `docker network connect` 进来，二者按容器名无缝直连。这从根本上绕开了通过 host-gateway 或暴露宿主机端口时极易被系统防火墙拦截导致的连接超时问题。基础服务自身保持网络无关——它的 `settings.conf` 不固化网络名，由上层负责动态组网。
