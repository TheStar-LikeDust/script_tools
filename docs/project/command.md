...
| 命令 | 行为约定 |
| :--- | :--- |
| `init` | 仅生成配置（渲染 `settings.conf`；集合服务还会初始化各附带依赖），不启动容器。结束时必须回显“下一步操作指引”（如提醒修改凭证或密码）。 |
| `start` | 读取已有配置启动容器（容器不存在用 `docker run` 新建，已存在用 `docker start`；集合服务按依赖顺序逐个拉起）。成功后必须高亮回显访问地址、端口及关键默认账号/密码。 |
