...
- `tools/`：宿主机（非 docker 级）的开发环境与工具（如 code-server、tmux）。该层不受 command.md 的服务部署命令规范强约束，可按工具自身需要设计命令；若存在配置落盘、实例隔离或生命周期管理需求，可复用 `init/start/up/stop/rm/purge`、`--conf/--name` 等通用约定。
